...
import useMessageQueries from "../hooks/useMessageQuries.js";
export default function productRoute(app){
    
    const {getMessages, createMessage, getMessage} = useMessageQueries()
  ...
  app.post("/api/comment/:id", verifyRequest(app), async (req, res) => {
    ...
    const response = await createMessage({product_id: params[1], description: params[0], to: params[2], user_name: "test", date: new Date()} );
